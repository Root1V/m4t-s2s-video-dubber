...
# Género del hablante para la voz de síntesis.
# Opciones: "male" (hombre), "female" (mujer), "auto" (detección automática)
# NOTA: Si auto falla, cámbialo manualmente aquí.
SPEAKER_GENDER = "male"
# ---------------------
...
def main():
    if not os.path.exists(ARCHIVO_ENTRADA):
        print(f"❌ Error: No se encontró '{ARCHIVO_ENTRADA}' en esta carpeta.")
        return

    # Nombre de salida con timestamp para no sobreescribir
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    archivo_salida = f"traducido_esp_{timestamp}.wav"
    ...
    if SPEAKER_GENDER == "auto":
        ...
        speaker_id = 0
        print("\n🎤 Género configurado manualmente: Hombre (speaker_id=0)")
    else:
        speaker_id = 1
        print("\n🎤 Género configurado manualmente: Mujer (speaker_id=1)")
